...
const PhotosPage = (): JSX.Element => {
    ...
    useEffect(() => {
        const doLoad = async () => {
            ...
            console.log(`${filteredImages.length} / ${newImages.length} images need transcriptions`);
            setLoading(false);
        };
        doLoad();
    }, []);

    const getTranscription = useCallback(async (note: NoteImage, force = false) => {
        if (!user) return;
        ...
        try {
            ...
            let message = `Transcribing note from ${dayjs(note.projects[0].date).format("DD MMMM YYYY")} (${note.projects.map(p => p.name).join(", ")})`;
            ...
            while(attempt < 30) {
                ...
                const hasTranscription = testResponse.data.transcription?.tagline?.trim()?.length;
                if(hasTranscription && (testResponse.data.transcription.rawText !== startingTranscription || !force)) {
                    transcription = testResponse.data.transcription;
                    break;
                }
                await new Promise(r => setTimeout(r, 2000));
                attempt++;
                setCurrentMessage(`${message} - waiting for brain response (${2 * (30 - attempt)}sec to timeout)`);
            }
            if(transcription) {
    ...
    return (
        <Layout>
            <div className="flex flex-col">
                ...
                <ProgressBar className="w-full grid place-items-center border border-white border-opacity-20 rounded-[10px] py-[8px]" innerClassName="bg-blue-500 rounded-[10px]" progress={currentIndex / images.length}>
                    <div className="flex flex-col items-center">
                        <p className="m-0 text-[25px]">{loadingTranscription ? `${(currentIndex + 1)} / ${images.length}` : "READY"}</p>
                        <p className="m-0 text-[12px]">{loadingTranscription ? (currentMessage || "Initializing") : `${images.length} note pages to be transcribed`}</p>
                    </div>
                </ProgressBar>
                {loadingTranscription ? (
                    <Button className="text-[20px] px-8 py-4 bg-red-700 hover:bg-red-500 transition-all duration 300 w-[200px] mx-auto rounded-[10px] mt-[20px]" onClick={() => {
                        document.location.reload();
                    }}>
                        STOOOPPPP
                    </Button>
                ) : (
                    <Button className="text-[20px] px-8 py-4 bg-cyan-700 hover:bg-cyan-500 transition-all duration 300 w-[200px] mx-auto rounded-[10px] mt-[20px]" onClick={() => {
